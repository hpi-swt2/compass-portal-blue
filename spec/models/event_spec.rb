...
RSpec.describe Event, type: :model do
  it "has name, description, start_time, end_time, room, and recurrence rule" do
    ...
    expect(event.start_time).to eq(Time.zone.parse("2021-10-25 13:15:00"))
    expect(event.end_time).to eq(Time.zone.parse("2021-10-25 14:45:00"))
  ...
  context "without recurrence rule" do
    ...
    it "returns itself in calendar_events if it is in the time frame" do
      calendar_events = event.calendar_events(Time.zone.parse("2021-10-20 13:15:00"),
                                              Time.zone.parse("2021-10-30 14:45:00"))
    ...
    it "returns itself in calendar_events if it intersects the time frame" do
      early_calendar_events = event.calendar_events(Time.zone.parse("2021-10-20 13:15:00"),
                                                    Time.zone.parse("2021-10-25 14:00:00"))
      late_calendar_events = event.calendar_events(Time.zone.parse("2021-10-25 14:00:00"),
                                                   Time.zone.parse("2021-10-30 14:45:00"))
    ...
    it "returns nil if it does not intersect the time frame" do
      earlier_calendar_events = event.calendar_events(Time.zone.parse("2021-10-20 13:15:00"),
                                                      Time.zone.parse("2021-10-22 14:45:00"))
      later_calendar_events = event.calendar_events(Time.zone.parse("2021-10-30 13:15:00"),
                                                    Time.zone.parse("2021-11-25 14:45:00"))
  ...
  context "with recurrence rule" do
    ...
    it "generates calendar events for all occurences in a time frame" do
      calendar_events = event.calendar_events(Time.zone.parse("2021-10-20 00:00:00"),
                                              Time.zone.parse("2022-01-25 00:00:00"))
      expect(calendar_events.length).to eq 3
      occurence_start_times = ["2021-10-25 13:15:00", "2021-11-01 13:15:00", "2021-11-08 13:15:00"].map do |string|
        Time.zone.parse(string)
      end
      occurence_end_times = ["2021-10-25 14:45:00", "2021-11-01 14:45:00", "2021-11-08 14:45:00"].map do |string|
        Time.zone.parse(string)
      end
