...
describe 'User authorization', type: :feature do
  it "is possible for an Admin can manage location and building" do
  ...
  it "is not possible for user without Admin permission to manage location and building" do
  ...
  it "is possible as user without Admin permission to update and delete an own location and building" do
    @user = create(:user, admin: false)
    ability = Ability.new(@user)
    expect(ability).to be_able_to(:manage, Location.new(users: [@user])) # location
    expect(ability).to be_able_to(:manage, Building.new(users: [@user])) # building
